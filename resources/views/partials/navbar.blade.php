        ...
        <div class="col-lg-4 top-brand">
          <a class="logo_h" href="{{ route('home') }}">
            ...
            <a title="{{$param->nombre_tienda}}" href="{{route('welcome')}}">
              <img alt="{{$param->nombre_tienda}}" src="//{{$param->logo}}" class="img-fluid logo-bar">
            </a>
          </a>
        </div>
...
@if(count(\App\Categoria6::all()) )
 ...
 @foreach($data->categories($allFilters) as $category)
                        <li class="nav-item">
                          <a class="nav-link text-capitalize" href="{{route('store.search', ['search' =>$category->nombrecategoria , 'strict' => true ])}}" class="no-decoration text-capitalize">{{$category->nombrecategoria}}</a>
                        </li>
             @endforeach
                      </ul>
                      @endif
